...
对于 macOS，这个选择尤其重要。当前项目没有直接实现 Visual Basic 到 macOS 原生机器码或 `Mach-O` 目标文件的完整后端，因此优先借用系统已经很成熟的 `clang/cc` 工具链，把 Portable C 当作中间代码。这样可以避免第一代就处理：
...
所以这个仓库仍然是 VB 编译器，只是当前采用的是“VB 前端 + C 后端”的实现策略。
...
它的职责不是解析 C，也不是把项目变成 C 编译器，而是把当前支持的 VB 子集稳定映射到 C，作为第一代可落地、可构建、可测试的后端。
